...
@click.option(
    ...
    "Requires exiftool be installed and in the path. "
    "This is useful mainly for iPhoto libraries. "
    "When an image is rotated in iPhoto, the image orientation is not actually changed, "
    "instead the image is tagged with an orientation value in the iPhoto database. "
    "This means that when the image is exported, the orientation may not be correct. "
    "This option will read the EXIF orientation data and fix the image's orientation if necessary. "
    "If used with Photos libraries, this option will result in the original image also being adjusted upon export. ",
)
@click.option(
    "--preview",
    is_flag=True,
